...
Firstly, `cd server`, type `yarn install`, after that type `yarn start` this is to run the server first which should   run on "http://localhost:3001/products".

Then you `cd test`, type `yarn install`, after that type `yarn start`, this is the client, which should run on "http://localhost:3000".